...
public class EventKick extends ListenerAdapter {
	@Override
	public void onGuildMemberRemove(@NotNull GuildMemberRemoveEvent event) {
		...
		if (!event.getGuild().getTextChannelsByName(channelName, true).isEmpty())
			event.getGuild().getTextChannelsByName(channelName, true).get(0).sendMessage(event.getMember().getAsMention() + " got kicked T_T").queue();
		else
			return;
